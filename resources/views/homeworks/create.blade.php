    ...
    <div class="container">
        <div class="card">
            ...
            <div class="body-card">
                ...
                <form method="post" action="{{ route('homeworks.store') }}" class="col-md-8 mb-2">
                    @csrf
                    <div class="form-group">
                        <label for="task">@lang('messages.placeholder.title')</label>
                        <input name="task" type="text" class="form-control" id="task" placeholder="@lang('messages.placeholder.task')" value="{{ old('task') }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="subject_id" class="form-control" id="subject">
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}" @if($subject->id == old('subject_id')) selected @endif>{{ $subject->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
