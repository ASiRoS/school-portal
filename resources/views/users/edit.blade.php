...
@section('content')
    <div class="container">
        <div class="card">
            <div class="card-header">@lang('messages.edit.users')</div>
            <div class="body-card">
                @include('partitions.errors')
                <form method="post" action="{{ route('users.update', ['user' => $user]) }}" enctype="multipart/form-data" class="col-md-6 mb-2">
                    @csrf
                    @method('put')
                    ...
                    <div class="form-group">
                        ...
                        <select name="role" class="form-control" id="role">
                            @foreach($roles as $role)
                                <option value="{{ $role }}" @if($role == $user->role) selected @endif>@lang('messages.roles.'.$role)</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group d-none" id="role-group">
                        ...
                        <select name="class_id" class="form-control" id="class">
                            ...
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" @if($class->id == $user->class_id) selected @endif>{{ $class->name }}</option>
                            @endforeach
                        </select>
                    </div>
