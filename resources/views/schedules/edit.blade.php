    ...
    <div class="container">
        <div class="card">
            ...
            <div class="body-card">
                ...
                <form method="post" action="{{ route('schedules.update', ['schedule' => $schedule]) }}" class="col-md-6 mb-2">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="subjects[]" class="form-control" id="subjects" multiple>
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}" @if($schedule->hasSubject($subject)) selected @endif>{{ $subject->title }}</option>
                            @endforeach
                        </select>
                    </div>
